...
def main():
    ...
    while True:
        ...
        _safe_json(metrics_data, metrics_file)
        _safe_json({"best_acc": float(best_acc) if not math.isnan(float(best_acc)) else None, "best_loss": float(best_loss) if not math.isnan(float(best_loss)) else None, "epoch": j, "task": args.task}, f"{log_dir}/best_info.json")
